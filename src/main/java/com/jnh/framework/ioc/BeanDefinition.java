...
public class BeanDefinition<T> {
    ...
    /**
     * [생성자 1] 클래스 기반
     * 클래스의 생성자를 호출해서 빈을 만드는 경우.
     * 빈 이름은 클래스명의 첫 글자를 소문자로 변환해 사용한다.
     * <p>
     * 예) UserService.class -> beanName: "userService"
     */
    ...
    public BeanDefinition(Class<?> configClass, String beanName) {
        Method method = Arrays.stream(configClass.getMethods())
    ...
    public String[] getBeanNames() {
        return ClsUtil.getParameterNames(cls, Autowired.class);
    }
    ...
    private T createBeanByMethod(ApplicationContext applicationContext) {
        ...
        return (T) method.invoke(applicationContext.getBean(configBeanName), parameters);
    }
